...
/**
 * Generic consumer that logs all TaskEvent messages. Useful for demos, debugging, and teaching how
...
public class TaskEventLoggingConsumer {
    ...
    private void handleTaskCreated(TaskEvent event) {
        ...
        log.info("[Event] TASK_CREATED — taskId={}, ownerId={}, title={}", p.getTaskId(), p.getOwnerId(), p.getTitle());
    }

    private void handleTaskUpdated(TaskEvent event) {
        var p = event.getPayload();
        log.info(
                "[Event] TASK_UPDATED — taskId={}, status={}, priority={}",
                p.getTaskId(),
                p.getStatus(),
                p.getPriority());
    }
    ...
    private void handleTaskCompleted(TaskEvent event) {
        var p = event.getPayload();
        log.info("[Event] TASK_COMPLETED — taskId={}, completedAt={}", p.getTaskId(), p.getUpdatedAt());
    }
    ...
    private void handleTaskRemoved(TaskEvent event) {
        ...
        log.info("[Event] TASK_REMOVED — taskId={}, ownerId={}, reason={}", p.getTaskId(), p.getOwnerId(), reason);
