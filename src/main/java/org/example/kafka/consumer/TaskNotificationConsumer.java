...
/**
 * Consumer responsible for handling user-facing notifications. In a real system, this would send
 * emails, push notifications, Slack messages, or in-app alerts.
 */
@Service
@Slf4j
public class TaskNotificationConsumer {
    ...
    private void handleTaskCreated(TaskEvent event) {
        ...
        log.info(
                "[Notification] Task CREATED → Notify user {} (taskId={}, title={})",
                payload.getOwnerId(),
                payload.getTaskId(),
                payload.getTitle());

        // Example:
        // notificationService.sendTaskCreatedEmail(payload.getOwnerId(), payload.getTaskId(),
        // payload.getTitle());
    }
    ...
    private void handleTaskCompleted(TaskEvent event) {
        var payload = event.getPayload();
        log.info(
                "[Notification] Task COMPLETED → Notify user {} (taskId={})",
                payload.getOwnerId(),
                payload.getTaskId());
    ...
    private void handleTaskRemoved(TaskEvent event) {
        var payload = event.getPayload();
        log.info("[Notification] Task REMOVED → Notify user {} (taskId={})", payload.getOwnerId(), payload.getTaskId());

        // notificationService.sendTaskRemovedAlert(payload.getOwnerId(), payload.getTaskId(),
        // payload.getRemovalReason());
    }
}
